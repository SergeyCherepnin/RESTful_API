...
- попытался подключить кэш(добавив аннотацию @EnableCaching к классу ResTfulApiApplication и в сервисе повесив над методами аннотацию @Cacheble). Попытка оказалась неудачной. Пробовал другие аннотации для методов: @CachePut - тоже неудачно.
- не получилось с передачей файла в тело запроса. Точнее не смог реализовать вариативность запроса в контроллере: либо путь к файлу(json), либо сам файл(MultipartFile) непосредственно.

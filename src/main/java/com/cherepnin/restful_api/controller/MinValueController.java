...
public class MinValueController {
    private final RestfulApiService restfulApiService;

    public MinValueController(RestfulApiService restfulApiService) {
        this.restfulApiService = restfulApiService;
    }

    @Tag(name = "Min value", description = "Min value definition")
    @Operation(requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
            ...
            responses = {@ApiResponse(responseCode = "200",
                    ...
                    content = @Content(
                            examples = {@ExampleObject(
                                    name = "min value",
                                    value = "{\"min_value\": \"-49999996\"}")}))})
    @PostMapping(value = "/get_min_value",
            produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    public ResponseEntity<?> getMinValue(@RequestBody FilePath filePath) {
        if (filePath == null)
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);

        HashMap<String, String> map = new HashMap<>();
        map.put("min_value", restfulApiService.getMinValue(filePath).toString());
